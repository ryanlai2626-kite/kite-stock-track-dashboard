    ...
    try:
        ...
        for ticker, name in indices.items():
            try:
                ...
                # 優先使用 fast_info (抗封鎖)
                try:
                    price = float(stock.fast_info.get('last_price'))
                    prev_close = float(stock.fast_info.get('previous_close'))
                except:
                    price = 0.0
                    prev_close = 0.0
                
                # 如果 fast_info 失敗，退回 history (5日K)
                if price <= 0:
